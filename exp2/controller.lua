...
local polygon_sides = 4


local RECORDING_TIME = 1900
local demo_repeat_target = 50

local state_demo = "signal_start"
local state_learner = "waiting"
local step_demo = 0
local visit = 0
local recording_steps = 0
...
TARGET_DIST = 60 -- the target distance between robots, in cm
...
-- local record_delay = 100
local trajectory_observed = {}
...
local visited_teachers = {} -- key: teacher_id, value: true
local segments_memory = {} -- { [teacher_id] = segments }
...
local segments_observed_original = {}
...
function step()
    ...
    elseif role == "learner" then
        ...
        if state_learner == "waiting" then
            ...
            -- log(string.format("[DEBUG] Range and bearing messages received: %d", #robot.range_and_bearing))
            for i = 1, #robot.range_and_bearing do
                local msg = robot.range_and_bearing[i]
                if msg.range <= approach_distance and msg.data[1] == 99 then
                    local teacher_id = msg.data[2] - string.byte("0")
                    -- log(string.format("[DEBUG] Found teacher %d with range %.2f", teacher_id, msg.range))
                    if not visited_teachers[teacher_id + 1] then
                        -- 只处理未访问的老师
                        current_teacher_id = teacher_id
                        log(string.format("[DEBUG] Approaching teacher %d with range %.2f", current_teacher_id,
                            msg.range))
                        closest_rb = msg
                    end
        ...
        if state_learner == "recording" then
            recording_steps = recording_steps + 1
            robot.leds.set_all_colors("yellow")
            -- ApproachDemonstrator()
            FollowDemonstrator(current_teacher_id)
            if not is_following then
                RecordObservation()
            end
            
            if recording_steps >= RECORDING_TIME then
                state_learner = "analyzing"
                recording_steps = 0
            end
        end
        if state_learner == "analyzing" then
            -- table.insert(observation_positions, observation_positions)
            robot.leds.set_all_colors("yellow")
            ...
            segments_observed_original = LinerRegression(largest_cluster)
            score_teacher = ScoreSegmentsStability(segments_observed_original)
            ...
            for i, seg in ipairs(segments_observed_original) do
                log(string.format("[O_SEG_original %d] angle = %.4f, distance = %.2f", i, math.deg(seg.angle), seg.distance))
            end

            SaveSegmentsToFile("segments_observed.txt", segments_observed)
            ...
            -- local quality = ComputeImitationQuality(segments_demo, segments_observed)
            -- local f = io.open("quality_scores.txt", "a")
            -- f:write(string.format("%.4f\n", quality))
            -- f:close()
            local f = io.open("teachers_scores.txt", "a")
            f:write(string.format("%d,%.4f\n", current_teacher_id, score_teacher))
            f:close()

            -- table.insert(teacher_quality_scores, quality)
            table.insert(teacher_quality_scores, score_teacher)

            visited_teachers[current_teacher_id + 1] = true
            segments_memory[current_teacher_id + 1] = segments_observed
            SaveTrajectoryToFile("trajectory_observed_" .. current_teacher_id + 1 .. ".txt", largest_cluster)
            segments_observed = {}
            largest_cluster = {}
            if visit < 3 then
                state_learner = "waiting"
            else
                log(string.format("[DEBUG] Imitation quality scores: %s", table.concat(teacher_quality_scores, ", ")))
                -- 选择最好的那个老师，进行模仿
                local best_teacher_id = nil
                local best_quality = -1
                for i, quality in ipairs(teacher_quality_scores) do
                    if quality > best_quality then
                        best_quality = quality
                        best_teacher_id = i
                    end
                end
                current_teacher_id = best_teacher_id
                log(string.format("[DEBUG] Best teacher is %d with quality %.4f", current_teacher_id, best_quality))
                segments_observed = segments_memory[current_teacher_id]
        ...
        if state_learner == "done" then
            SaveTrajectoryToFile("trajectory_total_observed.txt", trajectory_observed)
         
            log(string.format("[DEBUG] Imitation completed, best teacher is %s with quality %.4f", current_teacher_id,
                teacher_quality_scores[current_teacher_id]))
...
function PolygonMotion(polygon_sides)
    ...
    if state_polygon == "forward" then
        if robot.id == "demo0" then
            robot.wheels.set_velocity(velocity, velocity)
        elseif robot.id == "demo1" then
            robot.wheels.set_velocity(velocity + noise_turn, velocity)
        elseif robot.id == "demo2" then
            edge_steps = edge_length + math.random(10) * noise_length / (velocity) * ticks_per_second
            robot.wheels.set_velocity(velocity + (math.random(10) - 5) * noise_turn,
                velocity - (math.random(10) - 5) * noise_turn)
        end
        ...
        elseif robot.id == "demo1" then
            robot.wheels.set_velocity(turn_speed + noise_turn, -turn_speed - noise_turn)
        elseif robot.id == "demo2" then
            robot.wheels.set_velocity(turn_speed + (math.random(10) - 5) * noise_turn,
                -turn_speed + (math.random(10) - 5) * noise_turn)
        end
        if step_count_polygon >= turn_steps then
            ...
            if side_count_polygon >= polygon_sides then
                ...
                if not is_exist_segments_demo and robot.id == "demo0" then
...
function DetectTurnPoints(points, threshold, window)
    local result = {}
    for i = window + 1, #points - window - 1 do
        local moving = false
    ...
    for i = 1, #corners + 1 do
        ...
        if end_idx - start_idx >= 1 then
            ...
            if distance > 3.0 then
                ...
                table.insert(simplified_points, {
                    angle = angle,
                    distance = distance
                })
            end
...
function ApproachDemonstrator()
    local closest_rb = nil
    local min_range = approach_distance

    for i = 1, #robot.range_and_bearing do
        local msg = robot.range_and_bearing[i]
        if msg.range < min_range then
            min_range = msg.range
            closest_rb = msg
        end
    end
    ...
    if closest_rb then
        local angle = closest_rb.horizontal_bearing
        local range = closest_rb.range

        -- Record observation point
        local x = range * math.cos(angle)
        local y = range * math.sin(angle)
        table.insert(observation_positions, {
            x = x,
            y = y
        })
        ...
        -- Follow logic
        if range > 100.0 then
            local speeds = ComputeSpeedFromAngle(angle)
            robot.wheels.set_velocity(speeds[1], speeds[2])
        elseif range < 60.0 then
            robot.wheels.set_velocity(0, 0)
        end
    else
        RandomWalk()
...
function ComputeLennardJones(rho)
    local delta = TARGET_DIST
    local epsilon = EPSILON
    if rho == 0 then
        return 0
    end
    local ratio = delta / rho
    return -4 * epsilon * (math.pow(ratio, 4) - math.pow(ratio, 2)) / rho
end
...
function FollowDemonstrator(current_teacher_id)
    for i = 1, #robot.range_and_bearing do
        local rb = robot.range_and_bearing[i]
        if rb.data[2] - string.byte("0") == current_teacher_id then
        ...
        -- if rb.range > 100.0 and rb.data[1] == 99 and not visited_teachers[rb.data[2] - string.byte("0") + 1] then
        --     is_following = true
        --     demo_range = rb.range
        --     demo_angle = rb.horizontal_bearing
        --     break
...
function RandomWalk()
    ...
    if rep_len > 0.2 then
        if rep_angle > 0 then
            robot.wheels.set_velocity(turn_speed + 2, -turn_speed + 2) -- turn right
        else
            robot.wheels.set_velocity(2 - turn_speed, turn_speed + 2) -- turn left
        end
    else
...
function RemoveStaticRepeatsAndJumps(points, repeat_eps, jump_dist)
    if #points < 3 then
        return points
    end

    local filtered = {}
    local last_kept = points[1]
    table.insert(filtered, last_kept)
...
function ScoreSegmentsStability(segments_observed)
    ...
    local len_mean = mean_list(lengths)
    ...
    local turn_mean = (#turns > 0) and mean_list(turns) or 0.0
